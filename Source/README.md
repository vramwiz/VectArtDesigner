...
グラデーションは図形の塗り・線・文字の3対象でいったん完成（2026-09-09）。今後の追加は以下の責務へ配置し、公開窓口へ描画や属性変換を集積しない。
...
| Persistence/Svg/Paint | SVGペイント定義・参照、標準表現のない方式の表示PNG |
...
MIFの依存方向は公開変換APIからReader／Raster／Paint／Placementへ向ける。ReaderとPNG描画は公開変換APIへ依存しない。SVGのオブジェクト書出しはPaintWriterへ定義生成を委譲する。描画用シェーダーはUIや永続化へ依存しない。
