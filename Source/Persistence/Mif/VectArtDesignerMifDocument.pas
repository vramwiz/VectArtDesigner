...
uses
  ...
  VectArtDesignerMifPngMetadata, VectArtDesignerFillPaint,
...
function CreateFillTexturePng(Color: TColor; const Fill: TVectArtFillStyle): TBytes;
...
begin
  ...
  if not (Fill.Kind in [vfkLinearHorizontal,vfkLinearVertical]) then
  ...
  SetFillPaint(Paint,Color,Fill,RectF(0,0,64,64),1);
  ...
  AddWadaString(Result,'texture object type','gradation linear');
...
begin
  ...
  if TryReadPngString(Png,'waDA','texture object type',Kind) and
    SameText(Kind,'gradation linear') then
  begin
    C := 0; Angle := 0;
  ...
  try
    ...
    begin
      ...
      if ((Layer is TVectArtRectangleLayer) and
        (TVectArtRectangleLayer(Layer).FillStyle.Kind in [vfkRadial,vfkTexture])) or
        ((Layer is TVectArtPathLayer) and
        (TVectArtPathLayer(Layer).FillStyle.Kind in [vfkRadial,vfkTexture])) then
        Report.AddIssue(meikUnsupported,I,Layer.Name,
          '放射グラデーション・画像テクスチャのMIF属性は未検証です。SVGへ保存してください。');
