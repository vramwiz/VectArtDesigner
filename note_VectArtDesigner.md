...
- レイヤーは実行中に不変の`LayerId`、フラットな`GroupId`、描画内容ごとの`Revision`を持つ。
  ...
  所属関係は変更しない。
  サムネイル背景は市松模様を既定とし、一覧の`ThumbnailBackground`設定で白へ戻せる。
...
## 設定カテゴリの単一ページ化（2026-09-09）

- カテゴリアイコンによる1ページ切替を廃止。既存のTVectArtSettingsPanelと内部の設定UI・編集処理は維持し、TVectArtSettingsSectionsを全体の縦スクロール領域として再構成した。
- 利用可能な各カテゴリを既存順のままalTopで積み、カテゴリ名の見出しとBevelWidth=1のホストで区切る。図形では線／線の色／塗り色／影／情報、文字では文字色／情報／文字／縁取り／効果を同時表示する。
- 各パネルは表示中の子コントロール下端から内容高を決める。影と未対応装飾は既存のalClient子パネルを維持するため、必要な固定内容高を与える。外側だけをスクロールし、個別ページのスクロールバーは表示しない。
- 塗り色パネルを文字へ使う際は、積み上げ見出しも「文字色」へ同期する。選択変更後の再描画は従来の選択ページ1枚ではなく、表示中の全カテゴリへ適用する。
- MainFormLifecycleTestsで発見した親未接続エラーは、レイヤースクロールバーのUpdateScrollBarがドック接続前に走ることが原因。GetParentForm=nilの間は子HWND更新を行わず、接続後のResizeで同期するよう修正した。検査プロセスは以降非表示で実行する。
- SettingsUiTestsは利用可能な5パネルの同時表示、alTop、1pxベベル、非重複、旧アイコンの不存在、文字色見出し同期を検証。MainFormLifecycleTestsは図形／文字選択、全表示パネルの画素再描画、リサイズ、生成破棄3回を検証。両テスト通過。
- 外側スクロール領域でDoMouseWheelを処理し、縦スクロールバー上へマウスを置いた状態でも120px単位で移動するようにした。追加テストは行わず、本体ビルドのみ確認した。

## テンプレ図形の左ドック配置（2026-09-09）

- レイヤー一覧・Toolsと同じ左ドックへ、独立した「テンプレ図形」枠を追加。ToolIdはTemplates、既定幅145pxで、表示メニューから表示／非表示と既存のレイアウト編集による移動・フローティングを利用できる。
- 従来のツールウィンドウ形式だったTemplatePickerを埋め込み可能なTCustomControlへ変更。分類、枠のみ／塗りのみ／枠＋塗り、図形一覧、配置案内を保持し、幅に応じて列数を変える。
- テンプレ専用の色1／色2ラベル・色見本・編集ポップアップを廃止。一覧の枠色／塗り色と配置結果は、ツールパレット下部のEditorState.Color1／Color2を直接使用する。ツール側の星ボタンは現在のテンプレ配置ツールを選択する。
- EditorState変更時に新しいテンプレ枠も更新し、ツール下部の色変更を一覧へ即時反映する。本体ビルドのみ実施し、警告・エラー0。通常EXE更新済み。
- 新規TTemplatePanelFrameに対応するDFMリソースの追加漏れを修正。実行時の「リソースが存在しない」例外を解消し、本体ビルドのみ再確認して通常EXEを更新した。
- TemplatePickerのComboBox／Grid生成を、親枠のネイティブウィンドウが確立するCreateWndまで遅延した。親接続前にContextが設定されてもRefreshが未生成UIへ触れないようにし、「ウィンドウを持っていない」例外を解消。本体ビルド成功、起動中の通常EXEはロック中のため修正版はTestOutput/AppDebugへ出力した。
- テンプレ図形フレームの既定横幅を290pxから半分の145pxへ変更した。

## ポップアップメニューの重なり操作（2026-09-09）

- キャンバスとレイヤー一覧で共有するオブジェクト右クリックメニューへ「重なり」サブメニューを追加。最前面に移動、最背面に移動、前面に移動、背面に移動を配置した。
- 既存の複数レイヤー1段移動を再利用し、最前面／最背面は選択レイヤー群の相対順を保って端まで移動する。全移動を1個のCompoundCommandへまとめ、Undo／Redoを1操作として扱う。
- 現在位置とロック状態に応じて各項目のEnabledを更新する。本体ビルドのみ実施し、通常EXEを更新した。

## ポップアップメニューの整列操作（2026-09-09）

- 共通のオブジェクト右クリックメニューへ「整列」サブメニューを追加。左揃え、左右中央揃え、右揃え、上揃え、上下中央揃え、下揃え、中央揃えを配置した。
- 複数選択全体の見た目上の外接範囲を基準とする。回転した図形／文字、線、Bezierを含むPath、変形画像の外接範囲を計算し、各レイヤーを平行移動する。
- 全レイヤーの移動を1個のCompoundCommandへまとめてUndo／Redoに対応。単一選択、ロックを含む選択、未対応レイヤーを含む選択ではメニューを無効化する。本体ビルドのみ実施し、通常EXEを更新した。

## オブジェクトのクリップボード操作（2026-09-09）

- キャンバス／レイヤー一覧共通の右クリックメニュー先頭へ、切り取り、コピー、貼り付け、削除、複製を追加。Ctrl+X／Ctrl+C／Ctrl+V／Deleteは編集面フォーカス時の共通ショートカットへ接続した。
- コピー時は選択レイヤーだけを含む独自形式 `VectArtDesigner.Objects.v1` と、選択外接範囲を背景透明で描画したPNGを同じWindowsクリップボードへ格納する。一般アプリ向けBitmap形式も併記する。
- 貼り付け時は独自形式が正常なら図形、文字、線、Path、画像を編集可能なレイヤーとして復元する。元の重なり順とグループを保ち、グループIDを再割当てして24pxずらす。独自形式がない／破損時はPNG、Bitmap、DIBをImageとしてキャンバス中央へ配置する。
- 切り取りはクリップボード格納成功後だけ削除する。貼り付け、切り取り、削除はUndo／Redo対象。クリップボード占有中や破損画像はDocumentを変更しない。本体ビルドのみ実施した。
- 起動時のショートカット重複例外は、新規Delete登録が既存のレイヤー削除用Delete登録と重なったことが原因。新規登録だけを除去し、既存処理をそのまま使用するよう修正した。
- Image削除後のサムネイル同期エラーは、キャッシュ辞書が解放済みTVectArtImageLayer参照をキーとして保持していたことが原因。キーをTVectArtLayerIdへ変更し、削除後の照合とキャッシュ除去でオブジェクトを参照しないよう修正した。
- 文字列選択時は共通の「情報」セクションを文字色／文字／縁取り／効果より後の最下段へ送る。図形選択時は従来のカテゴリ順を維持する。
- オブジェクト右クリックメニューへ「オブジェクト属性の貼り付け」サブメニューを追加し、色、サイズ、区切り、文字、フォント名、縁取り、効果の順を再現した。選択的な属性適用コマンドは未実装なので、通常貼り付けと混同しないよう子項目は入口のみの無効表示とする。
- 文字設定の積み上げは、各ホストを一時的にalNoneへ戻してTopを「文字色、文字、縁取り、効果、情報」の順に設定した後、alTopへ復帰させる。表示構成・内容高・見出しが前回と同じ場合は再整列と全体再描画を行わず、文字入力通知によるちらつきを防ぐ。
- キャンバス上で文字編集中の文書変更通知はキャンバスだけ即時再描画し、レイヤー一覧・オブジェクト情報・編集状態・線ツールバーの更新を入力停止200ms後へ集約する。
- Ctrl+D／複製はRectangle、Line、Path、Image、Textの全レイヤー種別に対応する。閉じたPath、開いたPath、Bezier、異種レイヤーの混在選択を含め、全属性と相対順を保持し、グループIDを再割当てして24pxずらす。複製全体を1回のUndo／Redoとして扱う。

## レイヤー一覧のD&D（2026-09-09）

- 参照元は指定表記の入れ子パスではなく、実在する `D:/DelphiProg/test/SYNC_ScreenLayout/ScreenDesignMaker.dpr` と `Source/Layers/ScreenLayoutLayerList.pas` を使用した。
- 左ボタン押下後5px以上の移動でドラッグを開始し、対象行の上半分／下半分から文書上の挿入境界を決める。空白や行間では最寄り行を基準にする。
- ドラッグ中はアクセント色の挿入線をGDI／Direct2D双方で描画し、一覧の上下28pxでは50ms間隔で自動スクロールする。
- 既存の複数選択をクリックで崩さず、選択レイヤーを相対順を保った1ブロックとして移動する。フラットGroupIdのグループも分解せず一体で扱う。
- 変更前後のLayerId順を保持する専用コマンドで、D&D全体を1回のUndo／Redo対象とする。

## 完成時の責務分割とコメント整理（2026-09-09）

- `Core/Transfer/VectArtDesignerLayerDataTransfer.pas`を追加。ClipboardとLayerDuplicationに重複していたRectangle、Line、Path、Imageの完全なデータ取得と種類別削除を集約した。Path頂点とPNGバイト列は元レイヤーの寿命から独立させる。
...
- 2026-09-10再開時、大きな選択領域が共通レンダラーの16384px上限を超える問題を修正。出力を上限内へ縮小し、非四角形マスクの論理座標を出力倍率へ追従させた。CutoutClipboardTestsへ20000px領域を追加し、切り取り関連3テストを再確認した。

## オブジェクト属性の貼り付け（2026-09-10）

- 独自オブジェクト形式でコピーされた単一レイヤーを属性の取得元とし、色、サイズ、文字、フォント名を右クリックメニューから選択的に貼り付けられる。
- 色は図形／線／文字の同カテゴリ間だけに適用し、線幅、線種、塗りの有効状態、Opacityなど指定外の属性を維持する。サイズはRectangle／Text／Line／Path／Imageに対応し、各貼り付け先の中心を維持する。
- 文字とフォント名はText間だけに適用し、文字貼り付けではフォントとBounds、フォント名貼り付けでは文字、サイズ、装飾を維持する。複数選択への適用全体を1回のUndo／Redoにまとめる。
- 複数オブジェクトをコピーした場合、領域切り取りや外部画像だけがクリップボードにある場合、選択にロック済みレイヤーを含む場合は利用不可。縁取り／効果は対応モデルがないため無効表示を維持する。
- AttributePasteTestsと既存FillIntegrationTestsが通過し、Debug Win64本体も警告・エラー0でビルド済み。

## 切り抜き四角枠の表示修正（2026-09-10）

- 四角切り抜きの範囲機能とコピー結果は正常だったが、ドラッグ中／確定後の表示だけが欠けていた。透明Brushを指定した状態のFrameRectは枠を描かないため、ScreenOutlineを始点まで閉じ、GDI／Direct2Dとも点線PenによるPolyline描画へ統一した。
- 誤認に基づいて先行追加した切り抜きモード初期化と画像削除処理変更は取り消し、既存動作を維持した。CutoutSelectionTestsで四角輪郭が閉じていることを検証する。

## 最終レイアウト調整（2026-09-10）

- テンプレ図形は独立枠で常時選択できるため、Toolsの末尾にあった☆のテンプレ起動アイコンとイベント接続を削除した。ツールボタン数は12から11になり、作成色欄も1段上へ詰める。
- Layersの各行は可視／ロック操作と96pxサムネイルだけを残し、名称、寸法、種別、フォント、線幅、不透明度、グループ件数の文字表示を廃止した。詳細値はObjectProperties側で扱う。
- 左ドックの総幅を維持したまま、Layersの推奨幅を224pxから下部5ボタンが収まる170pxへ縮小し、空いた54pxをテンプレ図形枠へ渡して145pxから199pxへ拡大した。GroupThumbnailRendererTests、LayerGroupDisplayTests、Debug Win64本体ビルドが通過。

## キャンバス設定ダイアログ（2026-09-10）

- ダイアログ名、幅、高さ、プリセット、キャンセルを日本語表示へ変更した。幅・高さは現在値を初期表示する数値入力欄とし、プリセット外の任意サイズも入力できる。
- プリセット一覧でHD、フルHD、WQHD、4K、縦長、正方形などを選ぶと、対応する幅・高さが入力欄へ反映される。手入力が既存プリセットと一致した場合も一覧選択を同期する。
- 空欄、0、負数、整数外ではOKを無効化する。プリセット選択やダブルクリックではDocumentを変更せず、OKで閉じた場合にだけ入力値を採用する。Debug Win64本体ビルドが通過。

## 設定スライダーの連続更新（2026-09-10）

- Document変更通知ではキャンバスを最初に即時Invalidateするため、線の太さ、透明度、影などのスライダー操作結果は編集中もリアルタイム表示する。
- 共通のTHorizontalTrackBarControlにフォーカスがある変更だけ、レイヤー一覧、オブジェクト設定、線ツールバーなどの同期更新を既存の200msタイマーへ送る。操作が続けばタイマーを再始動し、入力停止後の1回へ集約するため、設定欄の値再設定・背景消去によるちらつきとドラッグ遅延を抑える。
- マウスドラッグだけでなく、ホイールとキーボードによるスライダー変更にも同じ処理を適用する。離散的な通常操作と選択変更は従来どおり即時同期する。SettingsUiTests、MainFormLifecycleTests、Debug Win64本体ビルドが通過。

## 線幅の下限（2026-09-10）

- オブジェクト設定の線の太さはスライダー範囲・数値入力範囲とも1～10000pxとし、スライダーを左端より外へ動かしても1pxへ補正する。適用処理側にも1px下限を設ける。
- 線そのものを消す操作は「枠と塗り潰し」の「塗りのみ」に限定し、そのモードが内部的にStrokeWidth=0を使う既存仕様は維持する。SettingsUiTestsへ0指定時のスライダー位置とDocument線幅の検証を追加し、Debug Win64本体ビルドが通過。

## ファイルメニューと新規作成（2026-09-10）

- ［ファイル］へ「新規キャンバス」「ウィザードで新規作成...」「開く...」「上書き保存」「名前を付けて保存...」「履歴」を配置した。Ctrl+Nは既定の1920x1080、Ctrl+Shift+Nはキャンバス設定ダイアログでOKにしたサイズから新規作成する。上部Newボタンも既定サイズの新規作成へ接続した。
- Document.ResetはDocumentインスタンスを交換せず、Canvas以外のレイヤー、選択、LayerId／GroupId採番を初期化する。キャンバスは白背景の指定サイズへ戻し、Undo／Redo履歴、MIFコンテナー、現在のファイル名、切り取り領域もMainForm側で消去する。新規Documentは未命名のため、最初の上書き保存では名前を付けて保存ダイアログを開く。
- SVG／MIFの読込または保存に成功したパスを最大10件、新しい順で［ファイル］→［履歴］サブメニューへ表示する。同じパスは先頭へ移動し、各項目から再度開ける。「履歴を消去」に対応し、MainForm.iniのFileHistoryセクションへ保存して次回起動時に復元する。
- FileActionsUiTestsでメニュー構造、新規作成通知、履歴の順序・重複・上限・クリック・INI往復、Document初期化を検証。MainFormLifecycleTestsで実際の新規キャンバスメニューをクリックし、既定サイズとレイヤー初期化を確認。Debug Win64本体ビルドは警告・エラー0。

## 未保存変更の表示と確認（2026-09-10）

- MainFormは最後に新規作成・読込・保存した時点のDocument.Revisionを保持する。内容Revisionが変わった時点でタイトルを`VectArtDesigner - ファイル名 *`または`VectArtDesigner - 新規キャンバス *`とし、選択だけの変更では`*`を付けない。
- SVG／MIFの保存成功後、読込成功後、新規キャンバス初期化後は未保存状態を解除する。Undo／Redo、キャンバスサイズ、各オブジェクト編集も通常のDocument変更通知を通るため未保存表示の対象となる。
- ウィンドウを閉じる、新規キャンバス、ウィザードで新規作成、開く／履歴から開く前に「変更を保存しますか？」を「はい／いいえ／キャンセル」で確認する。はいは現在ファイルへ保存し、未命名なら名前を付けて保存を開く。保存失敗、保存ダイアログのキャンセル、MIF確認のキャンセルでは破棄操作を続行しない。いいえだけ保存せず続行し、キャンセルは元のDocumentを維持する。
- ユーザー指示によりライフサイクルテストは実施せず、Debug Win64本体ビルドのみ行い警告・エラー0を確認した。

## 完成時のShell責務分割（2026-09-10）

- `Shell/Document/VectArtDesignerDocumentSession.pas`へRevisionによる未保存判定、表示名とタイトル、破棄前の保存確認を分離した。実際の保存は戻り値付きコールバックへ委譲し、ファイル形式やダイアログへ依存しない。
- `Shell/File/VectArtDesignerDocumentFileController.pas`へSVG／MIFの読込・保存、MIFコンテナーとReader／Writerの所有権、編集履歴のクリアを分離した。非対話テストでは警告・確認画面を表示せず、確認が必要なMIF変換を暗黙に承認しない。
- `Shell/File/VectArtDesignerRecentFiles.pas`へ履歴の重複排除、新しい順への移動、最大10件の制限、INIとの往復を分離した。FileActionsUIはメニューとファイルダイアログ、MainFormはイベント接続とステータス表示を担当する。
- MainFormは1321行から1129行へ縮小した。ユニット先頭コメントは目的と担当範囲、処理内コメントは非対話時の安全上の制約に限定し、コードの言い換えは追加していない。
- DocumentFileControllerTests、DocumentSessionTests、FileActionsUiTestsとDebug Win64本体ビルドが通過した。今回の整理ではライフサイクルテストを実施していない。

## キャンバス背景設定（2026-09-10）

- キャンバス設定ダイアログへ白・黒・透明・その他の色を追加し、既存のHSVカラーピッカーをダイアログ内へ常設した。透明選択中はピッカーを表示したまま無効化し、別フォームは開かない。
- サイズ、背景色、透明状態はOK時だけDocumentへ一括適用する。新規作成ウィザードにも同じ設定を使用し、通常の新規キャンバスは従来どおり白を既定とする。
- 透明キャンバスの編集表示は既存のDirect2D／GDI市松模様を使用する。市松模様は編集補助であり、保存画像へ含めない。
- MIF背景textureの既存`image alpha`を透明状態として読み書きする。背景色は`texture color1`、透明はalpha 0、不透明は255とし、アプリ固有キーは追加しない。
- ダークVCL Styleで標準Panelの色見本が黒く上書きされたため、色見本を既存の独自描画Swatchへ変更した。編集キャンバス外側は白・黒どちらのキャンバスとも境界が分かる中間灰色`#808080`とする。
- サイズプリセット一覧をドロップダウン式コンボボックスへ変更し、背景ピッカーを維持したままダイアログの高さを574pxから440pxへ縮小した。幅・高さの手入力と一致するプリセットへの同期は維持する。

## 上部ショートカットとレイヤー背景（2026-09-10）

- 上部ショートカットバーの高さを42pxから36pxへ縮小した。20pxのアイコンは固定した上下位置ではなく、バーの現在高から垂直中央を算出して描画する。
- レイヤー一覧の通常行・グループ行のサムネイル背景は市松模様を既定とする。既存の`ThumbnailBackground`プロパティと`vltbWhite`は残し、必要時に白背景へ戻せる。
- LayerRendererCanvasHiddenTestsとDebug Win64本体ビルドが通過した。

## 画像出力（2026-09-10）

- 編集データのMIF／SVGは「保存」、ラスタライズしたPNGは「出力」と区別する。［ファイル］→［出力］→［ファイル...／クリップボード］は表示中の全オブジェクトを対象にする。
- キャンバスまたはレイヤー一覧で選択オブジェクトを右クリックした［出力］→［ファイル...／クリップボード］は、現在選択中のオブジェクトだけを元の積層順で描画する。
- どちらの経路も出力寸法をキャンバス実寸とし、キャンバス外の描画を含めない。ファイルの種類はPNGを既定とし、GIFとJPEGも選択できる。PNGは透明キャンバスを透過のまま保存し、GIF／JPEGは半透明との互換性を優先して透明部分を白へ合成する。クリップボード出力はPNGとする。選択物のキャンバス外を含む純粋な範囲PNGと編集用JSONは、既存の［コピー］が引き続き担当する。
- レンダラーの実寸上限に合わせ、幅または高さが16384pxを超える場合は縮小せずエラーにする。PngOutputTestsで全体／選択の対象差、寸法、背景色、透明、ファイル保存、右クリックサブメニューを検証し、FileActionsUiTestsとDebug Win64本体ビルドも通過した。
- ［出力］サブメニューはファイルメニューと共通オブジェクトメニューの最下段へ置き、直前の区切り線で通常の保存・編集操作から分離する。

## 完成時のユニット整理（2026-09-10）

- `Editor/VectArtDesignerCanvas.pas`に同居していた作図中の破線分割、cap、端点マーカーのGDI／Direct2D描画を`Editor/Rendering/VectArtDesignerLinePreview.pas`へ分離した。Canvasは入力、表示キャッシュ、最終合成を担当し、1998行から1766行へ縮小した。
- `Shell/File/VectArtDesignerPngOutput.pas`は全体／選択の描画、背景合成、キャンバスクリッピング、PNGクリップボードを担当する。PNG／GIF／JPEGの形式判定、透明の白合成、エンコード、ディスク書込みは`Shell/File/VectArtDesignerImageFileEncoder.pas`へ分離した。
- Source配下のフォルダ別ユニット数を確認し、最大は6だった。既存の責務別サブフォルダで分散できており、大量同居するフォルダはないため、件数だけを理由にした移動は行っていない。
- 全Sourceユニットの先頭に目的と担当範囲のコメントがあることを確認した。欠けていた`VectArtDesignerAttributePasteOperations.pas`へ、属性貼付けとクリップボード読出しの責務境界を追記した。処理内コメントはGDIのアンチエイリアス制約、Direct2D状態復帰、透過非対応形式の白合成など、判断理由と制約に限定した。
- PngOutputTests、FileActionsUiTests、LayerRendererCanvasHiddenTests、Debug／Release Win64本体ビルドが通過した。
